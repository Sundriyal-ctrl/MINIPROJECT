...
<div class="shadow border" style="padding:20px;">
    ...
    <span>Source :  <%= application.getAttribute("source1") %></span><br>
    <span>Destination :  <%= application.getAttribute("destination1")%></span><br>
    <span>Price :  <%= application.getAttribute("price1")%></span><br>
    <span>Date :  <%= application.getAttribute("date1")%></span><br>
    <span>Invoice No  : <%=  randomInt %></span><br>
    <span>Invoice Date  : <% Date d=new Date();out.println(d); %></span><br>
    <span>Total Bill  : <%= application.getAttribute("price1")%></span>
    <br>
...
<input id="INDUSTRY_TYPE_ID"class="form-control"  readonly  name="INDUSTRY_TYPE_ID" autocomplete="off" value="Retail" type="hidden">
                    ...
                    cot = DriverManager.getConnection("jdbc:mysql://localhost:3306/ABC","root","");
   
  
                       PreparedStatement pt = cot.prepareStatement("select price from booking where id=?");
                       pt.setString(1,(String)application.getAttribute("id1"));
